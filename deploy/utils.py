...
def ndarray2str(str_array):
    mapping = {0: 'W', 1: 'N1', 2: 'N2', 3: 'N3', 4: 'R'}
...
def convert_sleep_stages(txt_path, output_xml_path, xml_template_path):
    """
    读取txt文件中的睡眠分期结果，并将其转换后写入新的XML文件中。
    映射关系为：{W: 10, N1: 1, N2: 2, N3: 3, R: 5}
    
    参数：
      txt_path: 包含睡眠分期结果的txt文件路径，每行代表一个分期（例如"W", "N1", …）。
      xml_template_path: XML模板文件路径，用于创建新的XML文件（见&#8203;:contentReference[oaicite:0]{index=0}）。
      output_xml_path: 新的XML文件保存的目标路径。
    """
    # 定义转换映射
    mapping = {"W": "10", "N1": "1", "N2": "2", "N3": "3", "R": "5"}
    
    # 读取txt文件中的每一行，并去除首尾空白字符
    with open(txt_path, "r", encoding="utf-8") as f:
        lines = f.read().splitlines()
    
    # 根据映射转换分期结果，忽略空行
    stages = [mapping[line.strip()] for line in lines if line.strip()]
    
    # 加载XML模板文件
    tree = ET.parse(xml_template_path)
    root = tree.getroot()
    
    # 查找SLEEPSTAGES节点，如果存在则清空其内容，否则创建该节点
    sleep_stages_element = root.find("SLEEPSTAGES")
    if sleep_stages_element is None:
        sleep_stages_element = ET.SubElement(root, "SLEEPSTAGES")
    else:
        sleep_stages_element.clear()
    
    # 根据转换后的结果添加新的SleepStage子节点
    for stage in stages:
        stage_element = ET.Element("SLEEPSTAGE")
        stage_element.text = stage
        sleep_stages_element.append(stage_element)
    
    # 将修改后的XML写入新的文件，保留XML声明和编码格式
