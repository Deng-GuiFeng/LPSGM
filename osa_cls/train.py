...
CLI entry point for APPLES OSA severity (binary) fine-tuning.

Delegates all training logic to ``cls_core.trainer.PooledTrainer``. This
module only defines argparse defaults specific to the APPLES OSA task and
...
def parse_args():
    p = argparse.ArgumentParser(description='OSA severity classification (APPLES, binary)')
    ...
    # -- Output --
    p.add_argument('--run_root', type=str, default='./run_osa')
