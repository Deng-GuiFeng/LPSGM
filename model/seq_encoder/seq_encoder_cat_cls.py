...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, input, mask, ch_idx, seq_idx, ori_len):
        ...
        SE = self.spatial_embedding[ch_idx]  # (bz*seql*cn, ch_emb_dim)
        TE = self.temporal_embedding[seq_idx]  # (bz*seql*cn, seq_emb_dim)
        ...
        cls_tokens = torch.cat([self.cls_token.unsqueeze(0).expand(bz, -1, -1), self.temporal_embedding.unsqueeze(0).expand(bz, -1, -1)], dim=-1)    # (bz, seql, hidden_dim+ch_emb_dim+seq_emb_dim)
        input = torch.cat([cls_tokens, input], dim=1)   # (bz, seql+seql*cn, hidden_dim+ch_emb_dim+seq_emb_dim)

        padding_mask = torch.zeros(bz, self.seq_len).bool().to(mask.device)
        mask = torch.cat([padding_mask, mask], dim=1)   # (bz, seql+seql*cn)
        ...
        feat = feat[:,:self.seq_len,:]   # (bz, seql, hidden_dim+ch_emb_dim+seq_emb_dim)
