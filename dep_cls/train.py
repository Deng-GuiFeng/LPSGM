...
CLI entry point for APPLES depression (binary) fine-tuning.

Delegates all training logic to ``cls_core.trainer.PooledTrainer``. This
module only defines argparse defaults specific to the APPLES depression
task and wires in the task's ``load_subjects`` implementation and label names.
"""
...
from .dataset import DEFAULT_DATA_ROOT, DEFAULT_LABEL_CSV, LABEL_NAMES, load_subjects


def parse_args():
    p = argparse.ArgumentParser(description='APPLES depression classification (binary)')
    ...
    # -- Data --
    p.add_argument('--label_csv', type=str, default=DEFAULT_LABEL_CSV)
    ...
    # -- Output --
    p.add_argument('--run_root', type=str, default='./run_dep')
...
def main():
    ...
    subjects = load_subjects(cfg.label_csv, cfg.data_root)
